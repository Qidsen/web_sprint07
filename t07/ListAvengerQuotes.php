...
class ListAvengerQuotes {
  protected $_list = [];
  public function addAvengerQuote($avengerQuote) {array_push($this->_list, $avengerQuote);}
  public function toXML($fileName) {
    ...
    foreach ($this->_list as $quote) {
      ...
      foreach((array)$quote->getPhotos() as $photo) {$xml_photos->addChild("photo", $photo);}
      ...
      foreach((array)$quote->getComments() as $comment) {
        ...
        foreach((array)$comment->getDate() as $date) {$xml_comment->addChild("date", $date);}
        foreach((array)$comment->getText() as $text) {$xml_comment->addChild("text", $text);}
      }
    }
  ...
  public function fromXML($fileName) {
    ...
    foreach($xml_obj->children() as $quotes) {
      ...
      $photos = [];
      foreach ((array)$quotes->photos as $photo) {$photos = $photo;}
      $avengerQuote = new AvengerQuote($id, $author, $quote, $photos);
      $comments = [];
      foreach ((array)$quotes->comments as $comment) {$comments = $comment;}
